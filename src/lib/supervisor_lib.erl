...
-export([start_dynamic_child/8,
	 make_child_id/2,
	 kill_dynamic_child/2,
	 apply_to_children/4]).
...
make_child_id(Id, Atom) ->
    list_to_atom(string:concat(string:concat(atom_to_list(Id), "_"),
			       atom_to_list(Atom))).
...
apply_to_children(Id, Filter, Arg, Fun) ->
    [Fun(Id, C) || {C, _, _, _} <- supervisor:which_children(Id),
		   Filter(C, Arg) == true].
